...
class ImagenDAO{
    ...
    public function save($imagen) {
        $this->conexion->conectar();
        $query = "INSERT INTO imagen (idImagen,idCasa,imagenPrincipal,nombreImagen,rutaImagen)"
                . " VALUES ( ".$imagen->getIdImagen()." ,  ".$imagen->getIdCasa()." ,  ".$imagen->getImagenPrincipal()." , '".$imagen->getNombreImagen()."' , '".$imagen->getRutaImagen()."' )";
        $result = $this->conexion->ejecutar($query);
        $this->conexion->desconectar();
        return $result;
    }
